...
from models.freight_p.air_freight.imp.euro.service import save_euro_pred_imp
...
from models.freight_p.sea_freight_index.ccfi_p.service import predict_ccfi
from models.freight_p.sea_freight_index.kcci_p.service import predict_kcci
from models.freight_p.sea_freight_index.hrci_p.service import predict_hrci
from models.freight_p.sea_freight_index.scfi_p.service import predict_scfi

# 해상운임 인덱스 예측 값 삽입 함수
from models.freight_p.sea_freight_index.bdi_p.service import insert_bdi
from models.freight_p.sea_freight_index.ccfi_p.service import insert_ccfi
from models.freight_p.sea_freight_index.kcci_p.service import insert_kcci
from models.freight_p.sea_freight_index.hrci_p.service import insert_hrci
from models.freight_p.sea_freight_index.scfi_p.service import insert_scfi
...
if __name__ == '__main__':
    ...
    try:
        sea_exp_path = 'models/insight_g/sea_freight/service_exp.py'
        subprocess.run([sys.executable, sea_exp_path])
    except Exception as e:
        logger.info('해상운임 수출 보고서 생성 또는 적재에 실패했습니다.')

    try:
        sea_imp_path = 'models/insight_g/sea_freight/service_imp.py'
        subprocess.run([sys.executable, sea_imp_path])
    except Exception as e:
        logger.info('해상운임 수입 보고서 생성 또는 적재에 실패했습니다.')

    try:
        air_exp_path = 'models/insight_g/air_freight/service_exp.py'
        subprocess.run([sys.executable, air_exp_path])
    except Exception as e:
        logger.info('항공운임 수출 보고서 생성 또는 적재에 실패했습니다.')

    try:
        sea_idx_path = 'models/insight_g/sea_freight_index/service.py'
        subprocess.run([sys.executable, sea_idx_path])
    except Exception as e:
        logger.info('해상운임지수 수출 보고서 생성 또는 적재에 실패했습니다.')