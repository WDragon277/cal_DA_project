...
from models.freight_p.sea_freight.repository import sea_freight_data
...
for (dptr_cnty, arvl_cnty), route_data in routes:
    ...
    # 예측값을 원래 스케일로 복원
    train_inv = scaler.inverse_transform(train.values.reshape(-1, 1))
    predictions_inv = scaler.inverse_transform(predictions.values.reshape(-1, 1))

    # RMSE 계산
    mae = mean_absolute_error(train_inv, predictions_inv)
    ...
    for i in range(1, 7):
        print(f"날짜: {train.index[i]}, 예측값: {predictions_inv[i][0]:.2f}")
    print("\n")
