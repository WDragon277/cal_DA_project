...
from models.ccfi_p.repository import ccfi_raw_data
...
model = AutoReg(train_data['ccfi_cach_expo'], lags=lag)
model_fit = model.fit()

predictions = model_fit.predict(start=len(train_data), end=len(train_data) + len(test_data) - 1)

RMSE = mean_squared_error(test_data['ccfi_cach_expo'], predictions)
r2 = r2_score(test_data['ccfi_cach_expo'], predictions)
...
plt.plot(test_date, test_data['ccfi_cach_expo'], color='red', linewidth=2, label='Actual')
