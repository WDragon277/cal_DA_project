...
for item_json, item_info in zip(dic_json, dic_info):

    logging.info(f'{item_json} start')

    try:
        item_json = gemma2.clean_json(item_json)
    except AttributeError:
        logging.info(f'{item_json}: AttributeError')
        pass

    content_list.append(gemma2.get_content(input_order=input_order,
                                               input_info=item_info,
                                               input_json=item_json)
                            )